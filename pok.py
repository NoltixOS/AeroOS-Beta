...
level = 30
pidgey = Pokemon('Pidgey', level, 25, 150)
eevee = Pokemon('Eevee', level, 175, 150)
pikachu = Pokemon('Pikachu', level, 325, 150)
pokemons = [pidgey, eevee, pikachu]

# the YOU's and OPPONENT's selected pokemon
YOU_pokemon = None
...
while game_status != 'quit':
            ...
            if game_status == 'select pokemon':
                ...
                for i in range(len(pokemons)):
                    ...
                    if pokemons[i].get_rect().collidepoint(mouse_click):
                        ...
                        # assign the YOU's and OPPONENT's pokemon
                        YOU_pokemon = pokemons[i]
                        OPPONENT_pokemon = pokemons[(i + 1) % len(pokemons)]
                        
                        # lower the OPPONENT pokemon's level to make the battle easier
                        OPPONENT_pokemon.level = int(OPPONENT_pokemon.level * .75)
                        
                        # set the coordinates of the hp bars
                        YOU_pokemon.hp_x = 275
                        YOU_pokemon.hp_y = 250
                        OPPONENT_pokemon.hp_x = 50
                        OPPONENT_pokemon.hp_y = 50
            ...
            # for selecting a move
                ...
                for i in range(len(move_buttons)):
                    ...
                    if button.collidepoint(mouse_click):
                        ...
                        YOU_pokemon.perform_attack(OPPONENT_pokemon, move)
    ...
    if game_status == 'prebattle':
        ...
        OPPONENT_pokemon.set_moves()
        
        # reposition the pokemons
        YOU_pokemon.x = -50
        YOU_pokemon.y = 100
        OPPONENT_pokemon.x = 250
        OPPONENT_pokemon.y = -50
        
        # resize the sprites
        YOU_pokemon.size = 300
        OPPONENT_pokemon.size = 300
        YOU_pokemon.set_sprite('back_default')
        OPPONENT_pokemon.set_sprite('front_default')
    ...
    if game_status == 'start battle':
        ...
        while alpha < 255:
            ...
            game.fill(white)
            OPPONENT_pokemon.draw(alpha)
            display_message(f'OPPONENT: {OPPONENT_pokemon.name}, i choose you!')
            alpha += .4
        ...
        # draw the hp bars
        YOU_pokemon.draw_hp()
        OPPONENT_pokemon.draw_hp()
        
        # determine who goes first
        if OPPONENT_pokemon.speed > YOU_pokemon.speed:
            game_status = 'OPPONENT turn'
        else:
    ...
    # OPPONENT selects a random move to attack with
    if game_status == 'OPPONENT turn':
        ...
        OPPONENT_pokemon.perform_attack(YOU_pokemon, move)
    ...
    if game_status == 'fainted':
        ...
        while alpha > 0:
            ...
            if OPPONENT_pokemon.current_hp == 0:
                YOU_pokemon.draw()
                OPPONENT_pokemon.draw(alpha)
                display_message(f'OPPONENT has lost the battle!')
            else:
                YOU_pokemon.draw(alpha)
                OPPONENT_pokemon.draw()
